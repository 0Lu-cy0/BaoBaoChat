 ...
 * Gửi lời mời kết bạn
 */
const sendFriendRequest = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
    ...
    try {
      ...
      await createNotification(
        ...
        "friend_request",
        "Lời mời kết bạn mới",
        `${req.user.display_name} đã gửi lời mời kết bạn`,
        friendRequest._id,
        req.user._id
      );
    } catch (notiError) {
 ...
 */
const getSentRequests = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: "Chưa đăng nhập" });
      return;
 ...
 */
const getReceivedRequests = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: "Chưa đăng nhập" });
      return;
 ...
 */
const acceptRequest = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(requestId)) throw new Error("INVALID_ID");
    ...
    if (!friendRequest) throw new Error("NOT_FOUND");
    if (!friendRequest.to.equals(req.user._id)) throw new Error("FORBIDDEN");
    ...
    try {
      ...
      await createNotification(
        ...
        `${req.user.display_name} đã đồng ý kết bạn`,
        friend[0]._id,
        req.user._id
      );
 ...
 */
const declineRequest = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: "Chưa đăng nhập" });
      return;
